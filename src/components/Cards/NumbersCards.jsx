...
const NumbersCards = ({key, id, header, metric, amount, percent, growthDirection}) => {
  return (
    <div key={key} id={id} className='bg-white py-3 rounded-eight pl-18px pr-3 flex flex-col justify-between'>
        <div>
        ...
        <div className='pt-3 flex flex-row items-end justify-between text-brandGreen4x'>
            ...
            <div className='flex items-center gap-2'>
                {growthDirection.toLowerCase() === 'up' ?
                ...
                <p className={`${growthDirection.toLowerCase() === 'up' ? 'text-brandGreen4x' : 'text-brandRed1x'} text-xs`}>{percent}</p>
            </div>
        </div>
    </div>
  )
