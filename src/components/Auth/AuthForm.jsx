    ...
    <form onSubmit={handleSubmit} className={`z-20 mx-auto md:mx-0 h-fit w-ninetyFivePercent sm:w-eightyPercent rounded-ten md:rounded-none md:w-auto md:static bg-white col-span-8 lg:col-span-4 px-4 sm:px-9 py-12 md:h-full`}>
        ...
        <section className='text-sm sm:text-base md:w-fit mx-auto sm:px-5 space-y-6 sm:space-y-8'>
            ...
            <label htmlFor={`${authPageType === 'signup' ? 'signUpEmail' : 'signInEmail'}`} className='bg-brandGray28x flex flex-col focus-within:outline focus-within:outline-1 focus-within:outline-brandGreen7x px-10 py-3 rounded-thirty'>
                <p  className='text-brandGray31x'>Your email</p>
                <div>
           ...
           {authPageType === 'login' && 
                <label htmlFor={`${authPageType === 'signup' ? 'signUpPassword' : 'signInPassword'}`} className='bg-brandGray28x flex flex-col focus-within:outline focus-within:outline-1 focus-within:outline-brandGreen7x px-10 py-3 rounded-thirty'>
                    <p  className='text-brandGray31x'>Your password</p>
                    <div className='relative'>
                        <input type={`${showPassword ? 'text' : 'password'}`} name="" id={`${authPageType === 'signup' ? 'signUpPassword' : 'signInPassword'}`} placeholder='' className='bg-transparent focus:outline-none w-full pr-8 authPassword'/>
                        <button type='button' onClick={togglePassword} className='absolute right-0 top-fiftyPercent -translate-y-fiftyPercent group'>
                            ...
                            <div className={`bg-brandGray31x group-hover:bg-brandGreen7x h-7 w-0.5 rotate-45 ${showPassword || 'hidden'} absolute right-3 top-fiftyPercent -translate-y-fiftyPercent`}></div>
                        </button>
                    </div>
                </label>
            }
           ...
           ? 
                <div className='font-avenirLight text-center'>
                    Already have an account? <a href='/login' className="text-brandGreen7x underline">Login</a>
                </div>
            :
                <div className='font-avenirLight text-center'>
                    Don't have an account? <a href='/sign-up' className="text-brandGreen7x underline">Create an account</a>
                </div>
            }
