...
const InnerPopPrRequest = ({id, user, index, ecoBagProducer, allRequests, dataLength, moveToModal, setCurrentIndex, address, state, avatar, handleAccept, handleReject, fullName, bagType, phoneNo, agentName, orderDate, availableProducers, handleRestrict, orderRequest, orderDateTime, orderStatus, orderQuantity, email, deliveryDate, contactPerson, contactInfo }) => {
...
return (
    <section id={id} className='bg-white z-50 rounded-ten w-ninetyFivePercent sm:w-eightyPercent lg:w-sixtyPercent h-fit m-auto'>
        ...
        <div className='rounded-b-ten px-5 sm:px-10 md:px-14 py-5 sm:py-10'>
            ...
            <Request handleApproval={()=>handleAccept(id)} handleReject={()=>handleReject(id)} buttons={ecoBagProducer ? false : true} paddingB={'pb-8'} paddingT={'pt-8'} reqName={'Eco-friendly bag production request'} actionBtnText={'Accept'} secBtnText={'Reject'} secBtnTextColor={''} actionBtnBgColor={'bg-brandBlue1x'} />

            
            {ecoBagProducer 
            ?
                <div className='py-5 border-b-0.5 border-b-brandGray27x overflow-x-auto'>
                    <TableEcoBagPop orderRequest={orderRequest} orderDateTime={orderDateTime} orderStatus={orderStatus} orderQuantity={orderQuantity} email={email} deliveryDate={deliveryDate} contactPerson={contactPerson} contactInfo={contactInfo} />
                </div>
            :
                <div className='grid grid-cols-1 sm:grid-cols-5 border-b-0.5 border-b-brandGray27x divide-y divide-x-0 sm:divide-x sm:divide-y-0 divide-brandGray27x overflow-x-auto'>
                    <div className='col-span-1 sm:col-span-3 py-5 sm:pr-2'>
                        <TablePopPrRequest fullName={fullName} bagType={bagType} phoneNo={phoneNo} agentName={agentName} email={email} deliveryDate={deliveryDate} orderDate={orderDate} contactInfo={contactInfo} orderQuantity={orderQuantity} availableProducers={availableProducers} />
                    </div>
