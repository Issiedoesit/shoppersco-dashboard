...
const Header = ({page}) => {
  ...
  return (
    <div className='bg-brandGray28x flex gap-2 items-center xl:block w-full py-7 sticky top-0 right-10 z-30'>
        <div className='rounded-ten py-3.5 xs:px-2 px-4 md:px-8 bg-white flex flex-row w-full items-center justify-between'>
            <h1 className='font-avenirHeavy sm:text-lg md:text-xl uppercase'>{page}</h1>
           ...
           <div className='flex flex-row items-center font-avenirHeavy gap-3 w-fit'>
            ...
            <button ref={menuRef} id='menu' onClick={handleNav} className={ `h-full ${navState && ''} transition-all duration-300 space-y-1 xl:hidden group`}>
              <div className={`bg-brandBlack1x w-8 h-0.75 rounded-ten group-hover:bg-brandGreen4x ${navState ? 'rotate-45 origin-center translate-y-[3.5px]' : ''} transition-all duration-300 ease-in-out`}></div>
              <div className={`bg-brandBlack1x w-8 h-0.75 rounded-ten  ${navState && 'hidden'} transition-all duration-100 ease-in-out`}></div>
              <div className={`bg-brandBlack1x w-8 h-0.75 rounded-ten group-hover:bg-brandGreen4x ${navState ? '-rotate-45 origin-center -translate-y-[3.5px]' : ''} transition-all duration-300 ease-in-out`}></div>
