...
const RowMainCampaign = ({id, avatar, performance, participants, startDate, endDate, rewardType, campaign, state, handleModalOpen}) => {


  return (
    <tr id={id} className='odd:bg-brandGray28x font-avenirLight text-sm state-insight-row'>
        ...
        <td className="py-3 pl-1 pr-3 whitespace-nowrap">
        <img src={avatar || dummyAvatar} alt="avatar" className='h-8 w-8 min-w-8 aspect-square min-h-8' />
        </td>
        <td className="py-3 pl-1 pr-3 whitespace-nowrap">
            <div className=''>
                <button onClick={handleModalOpen} type='button' className="text-xs underline hover:text-brandGreen7x">{campaign || '----'}</button>
